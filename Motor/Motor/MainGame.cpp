...
void MainGame::initLevel() {
	...
	// Inicialización de zombies
	zombiesEncontrados = 0;
	auto zombiesPositions = levels[currentLevel]->getZombiesPosition();
	for (size_t i = 0; i < levels[currentLevel]->getZombiesPosition().size(); i++) {
		zombies.push_back(new Zombie());
		zombies.back()->init(0.5f, levels[currentLevel]->getZombiesPosition()[i]); // Ajustar parámetros según sea necesario
		zombiesEncontrados++;
		std::cout << "Zombies encontrados: " << zombiesPositions[i].x << ", " << zombiesPositions[i].y << std::endl;
	}

	// Imprimir cantidad de zombies encontrados
	std::cout << "Zombies encontrados: " << zombiesEncontrados << std::endl;
...
void MainGame::update() {
	while (gameState != GameState::EXIT) {
		...
		for (size_t i = 0; i < zombies.size(); i++) {
			zombies[i]->update(levels[currentLevel]->getLevelData(), humans, zombies, player);
		}
